        ...
        if self.line:
            print(
                "  {}{}: {}{}{} : {}{}".format(
                    ...
                    self.line,
                    COLORS.get(color.upper()),
                    self.category,
                    category_right_padding,
                    COLORS.get("ENDC"),
                    self.message,
                )
            )
...
class Collecter:
    ...
    def display(self):
        logs = {"critical": False, "warning": False, "enhancement": False}
        ...
        if logs["critical"] or logs["warning"] or logs["enhancement"]:
            ...
            if App._args.get("json"):
                output = {
                    log_class: {c: {} for c in v.get("categories")}
                    for log_class, v in self.log_classes.items()
                }
                ...
                for log_class, v in self.log_classes.items():
                    ...
                    if logs.get(log_class):
                        print(
                            "{}{} :{}".format(
                                COLORS.get(color), log_class.upper(), COLORS.get("ENDC")
                            )
                        )
                        for log in v.get("logs"):
                            if log.category in categories:
                                category_right_padding = self.find_longest_category_name(
                                    log_class
                                ) - len(
                                    log.category
                                )
                                line_number_right_padding = self.find_highest_line_number() - len(
                                    str(log.line)
                                )
                                log.display(
                                    color,
            ...
            print(
                "{}{}{}\n".format(
                    COLORS.get("GREEN"), "Everything is good", COLORS.get("ENDC")
                )
            )
            exit(0)
